...
placement_config () {
	...
	ops_add $placementfile placement_database connection mysql+pymysql://placement:$PLACEMENT_DBPASS@$CTL_MGNT_IP/placement

	ops_add $placementfile api auth_strategy keystone

	ops_add $placementfile keystone_authtoken auth_url http://$CTL_MGNT_IP:5000/v3
	ops_add $placementfile keystone_authtoken memcached_servers $CTL_MGNT_IP:11211
