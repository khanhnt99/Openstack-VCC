...
glance_config_api () {
	...
	ops_add $glanceapifile database connection mysql+pymysql://glance:$GLANCE_DBPASS@$CTL_MGNT_IP/glance
	ops_add $glanceapifile keystone_authtoken auth_uri http://$CTL_MGNT_IP:5000	  
	ops_add $glanceapifile keystone_authtoken auth_url http://$CTL_MGNT_IP:5000
	ops_add $glanceapifile keystone_authtoken memcached_servers $CTL_MGNT_IP:11211	  
