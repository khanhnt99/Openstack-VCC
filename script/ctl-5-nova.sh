...
nova_create_info () {
	...
	openstack endpoint create --region RegionOne compute public http://$CTL_MGNT_IP:8774/v2.1
	openstack endpoint create --region RegionOne compute internal http://$CTL_MGNT_IP:8774/v2.1
	openstack endpoint create --region RegionOne compute admin http://$CTL_MGNT_IP:8774/v2.1
...
nova_config () {
	...
	ops_add $novafile api_database connection mysql+pymysql://nova:$NOVA_DBPASS@$CTL_MGNT_IP/nova_api

	ops_add $novafile database connection mysql+pymysql://nova:$NOVA_DBPASS@$CTL_MGNT_IP/nova

	ops_add $novafile DEFAULT transport_url rabbit://openstack:$RABBIT_PASS@$CTL_MGNT_IP

	ops_add $novafile api auth_strategy keystone

    ops_add $novafile keystone_authtoken www_authenticate_uri http://$CTL_MGNT_IP:5000
	ops_add $novafile keystone_authtoken auth_url http://$CTL_MGNT_IP:5000/v3
	ops_add $novafile keystone_authtoken memcached_servers $CTL_MGNT_IP:11211
	...
	ops_add $novafile glance api_servers http://$CTL_MGNT_IP:9292
	...
	ops_add $novafile placement auth_url http://$CTL_MGNT_IP:5000/v3
